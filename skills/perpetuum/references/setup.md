...
初始化只收集信息和建立 Harness，不负责安装或升级运行环境。先确认以下能力可用：
...
6. 检查信息是否足以支持长期自主运行。至少确认项目目录与名称、长期目标、当前背景与进度、可用资源与权限、边界与底线、进展证据、工作时间窗口，以及长期目标之外必须兼顾的要求。
7. 对无法可靠推断、互相矛盾或会改变行为的内容逐项询问。用户回答后重新整理完整摘要；若仍有缺口，继续询问和确认，不要因为已经进行过一轮问答就跳过剩余问题。
8. 向用户展示最终的 `goal.md`、初始 `plan.md`、`history.md` 摘要及运行配置。`goal.md` 是长期契约，`plan.md` 是 Task 列表，`history.md` 是已验证结论的精简历史。只有在用户明确确认这些内容无误后，才调用单一入口注册项目：
...
9. 注册命令会生成完整的 Harness 骨架。用确认后的内容替换其中的 `goal.md`、`plan.md`、`history.md`，再检查其他通信文件的标题和项目背景。
