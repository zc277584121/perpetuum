...
4. 检查异常旧状态是否可能表示仍有工作在运行。只有本次由自己创建并保存了精确名称的直属 session 才能操作；不能确认所有权时记录管控异常，不清理来源不明的 session，也不创建可能重复的下级工作。
...
- 当前承载它的 session 精确名称，并说明该 session 由 Root 管理，不得由它关闭、接管或发送消息；
...
- 结束前只关闭本次由自己明确创建并保存名称的直属 session。
...
2. 按当前 `cc-use` Skill 的退出流程关闭并复核本次明确创建且保存了精确名称的直属 Project Supervisor session。不要根据全局列表猜测并清理其他 session。
