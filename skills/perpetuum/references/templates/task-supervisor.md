...
Task Supervisor 负责一次 Task 选择、执行和验证循环。Explorer 选出 Task 后，它的生命周期就对应这个 Task；没有可执行 Task 时则以 Idle 结束。它不应该长期占据 Project 的唯一 Task 名额而没有明确状态。

## 标准流程

1. 读取 Goal、Plan、History 和尚未处理的人类输入。
2. 通过 `cc-use` 启动全新的 Explorer session，要求读取 [explorer.md](explorer.md)，维护候选 Task、优先级并选择本次唯一 Task。
...
5. 在 Executor 提供可验证结果后，启动与 Executor 分离的 Validator，要求读取 [validator.md](validator.md)。Validator 默认使用全新 session。
...
- Task Supervisor 自身对应本次工作循环。选出 Task 后，该 session 只服务这个 Task；新的 Task 使用新的 Task Supervisor session，不复用已经结束的上一次 Task 会话。
