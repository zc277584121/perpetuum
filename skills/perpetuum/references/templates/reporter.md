...
对所有项目完成初稿后，必须按以下顺序结束：

1. 对自己创建的每个直属 cc-use session 调用 `finish`，确认真实退出状态并复核没有遗留直属 session；不能把清理责任留给 Runner；
2. 根据最终退出与清理结果更新当天日报和 `latest.md`，不要让报告长期停留在“Reporter 仍在运行”的采样状态；
3. 复核两份报告内容一致，再按 Runner dispatch 原子写入完成回执；
4. 写完回执后等待 Runner 回收当前顶层 session，不再启动新的工作。
