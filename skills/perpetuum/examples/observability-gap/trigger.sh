...
ensure_middle_session() {
  if ! tmux has-session -t "$MIDDLE_SESSION" 2>/dev/null; then
    ...
    tmux new-session -d -s "$MIDDLE_SESSION" -c "$PROJECT_ROOT" 'claude --dangerously-skip-permissions'
    sleep "$TUI_BOOT_WAIT"
  fi
}

send_prompt() {
  tmux set-buffer -b obs_prompt "$1"
  tmux paste-buffer -t "$MIDDLE_SESSION" -b obs_prompt
...
run_cycle() {
  local cid="$1"
  for pf in $(ls "$TASK_DIR"/[0-9]*_*.md 2>/dev/null | sort); do
